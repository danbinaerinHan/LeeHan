...
기준은 **길이**다. 줄 수를 먼저 정하고 제목을 맞추는 게 아니다.

- **한 줄에 들어가면 `<br>`을 넣지 않는다.** 8자 안팎(공백 포함 10자 이하)이면 모바일 375px에서도
  한 줄로 들어간다 — `빛이 밤을 모를 때`는 상세 68px에서 428px / 모바일 36px에서 230px로,
  각각 652px · 327px 칼럼에 넉넉히 앉는다. 굳이 끊으면 짧은 조각 두 개로 쪼개져 되레 허전하다
- **넘칠 때만 두 줄로 끊는다.** `<br>` 없이 두면 어절 경계에서 알아서 접히지만, 접히는 위치가
  의미 단위와 어긋날 때 `<br>`로 못박는다 (`평면 조각가의` / `우연한 공간`, `니가가라` / `하와이 사진전`)
- 조사나 어절 중간에서 끊지 않는다
- 세 줄이 되면 `<br>` 위치를 바꾸기 전에 **제목 자체를 줄일 수 있는지** 본다.
  세 줄은 포스터와의 균형이 깨진다
- 줄 수를 바꿨으면 **상세(68px)와 모바일(36px) 양쪽에서 실제 폭을 재 본다.**
  `range.selectNodeContents(el)` 로 글자 폭을, `getBoundingClientRect().width` 로 칼럼 폭을 잰다
